...
To sample from this distribution, we define a function that takes two parameters, $\theta$ and $V^{-1} = \mathbf{W}^t \mathbf{R}^{- 1} \mathbf{W}+\mathbf{D}^{- 1})^{- 1}$. We note that for these type of matrices, the build in `solve` function won't work; design matrices tend to be singular. I frequently use the generalized inverse in `MASS`.
...
\sigma_{e_1}^2 |  \mathbf{\beta}, \mathbf{u}_1, \ldots,
      ...
      \ldots, \sigma_{u_{3 ( k)}}^2, \mathbf{y}, \mathbf{h} \sim Inv -
      Scaled -\mathbf{\chi}^2 ( \hat{\nu}_{e_i}, \hat{s}^2_{e_i})
$$

Using the `geoR` library for a scaled inverse $\chi^2$ distribution,
```{r}
library(geoR)
...
\sigma_{u_m}^2 |  \mathbf{\beta}, \mathbf{u}_1, \ldots,
      ...
      \mathbf{h} \sim Inv-Scaled -\mathbf{\chi}^2 (
