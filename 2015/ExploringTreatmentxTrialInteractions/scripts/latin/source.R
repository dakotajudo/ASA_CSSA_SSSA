...
setwd('C:/Users/peter/AppData/Local/Temp/ARM/2664/')
options(OutDec='.')
st.dat <- read.delim('C:/Users/peter/AppData/Local/Temp/ARM/2664/st.dat',header=TRUE)
st.dat$Trial <- as.factor(st.dat$Trial)
st.dat$Treatment <- as.factor(st.dat$Treatment)
st.dat$RepNo <- as.factor(st.dat$RepNo)
st.dat$ColNo <- as.factor(st.dat$ColNo)
tryCatch({
   res <- analyze.ARMST.latin(st.dat)
   aov.tbl <- tryCatch({
      means.tbl <- tryCatch({
   ...
   het0.tbl <- summary(het$heterogeneous0.lm)$coefficients
   het0.tbl <- extract.slopes(het0.tbl,het$heterogeneous0.lm$df.residual)
   write.table(het0.tbl,file='hetcoef.tab',row.names=TRUE,col.names=FALSE,sep='\t')
   }, error = function(e) {
   cat(geterrmessage(),file='R.err')
   cat("res <- analyze.ARMST.latin(st.dat)\r",file='R.out')
})
