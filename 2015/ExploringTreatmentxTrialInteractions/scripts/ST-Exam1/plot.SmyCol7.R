...
require(grDevices)
means.vector <- read.delim('trialMeans.SmyCol7.tab',header=FALSE)
means.matrix <- read.delim('trialTable.SmyCol7.tab',header=FALSE)
means.vector <- means.vector[,1]
bmp('TxTSmyCol7.bmp', width = 620, height = 500, units = 'px', pointsize = 12, bg = 'white')
par(fig = c(0, 1, 0.4, 1), mar = (c(1, 6, 2, 1) + 0.1), ps = 12, cex.lab = 1.166667, cex.main = 1.333333, cex.axis = 1)
plot.interaction.ARMST(means.matrix, means.vector, ylab='Treatment in Trial Mean \n% CONTRO',regression=TRUE, main='Treatment Stability and Trial Clusters for Grand Mean 7', show.legend=TRUE,legend.columns=1, legend.pos=c(.01,.98))
